...
@pytest.mark.parametrize(
    ...
    [
        ([1, 0, -1, 0, -2, 2], 0, [[-2, -1, 1, 2], [-2, 0, 0, 2], [-1, 0, 0, 1]]),  # Example 1
        ...
        ([1, 0, -1, 0, -2, 2], 1, [[-2, 0, 1, 2], [-1, 0, 0, 2]]),  # Mixed positive and negative numbers
    ],
)
def test_finds_correct_quadruplets(solution, nums, target, expected):
    result = solution.fourSum(nums, target)
    assert sorted([sorted(quad) for quad in result]) == sorted([sorted(quad) for quad in expected])